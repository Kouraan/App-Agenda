...
public class PaginaPrincipalController implements Initializable {
    ...
    @FXML
    private Label semanaLabel;
    @FXML
    private Button todayBtn;
    @FXML
    private Button semanaAnteriorBtn;
    @FXML
    private Button proximaSemanaBtn;
    @FXML
    private Button logoutBtn;
    @FXML
    private GridPane calendarioGrid;
    @FXML
    private ToggleButton semanaToggle;
    @FXML
    private ToggleButton mesToggle;
    @FXML
    private ToggleButton diaToggle;
    @FXML
    private ToggleButton calendarioToggle;
    @FXML
    private ToggleButton clientesToggle;
    @FXML
    private Label relogioLabel;
    @FXML
    private VBox areaCentral;
    @FXML
    private StackPane areaClientes;
    @FXML
    private VBox clientesContent;
    @FXML
    private ScrollPane scrollPaneCalendario;
    @FXML
    private VBox anotacoesBox;
    @FXML
    private BorderPane rootPane;
    @FXML
    private TextArea anotacoesArea;
    @FXML
    private Button blurToggleBtn;
    @FXML
    private VBox caixaClientesPendentes;
        ...
        group.selectedToggleProperty().addListener((obs, oldToggle, newToggle) -> {
            if (newToggle == semanaToggle) {
                ...
                if (oldToggle == mesToggle) {
                    ...
                    // Se o dia selecionado não pertence à semana atual, mostrar a semana
                    // correspondente ao dia selecionado
                ...
                if (oldToggle == mesToggle) {
                    ...
                    // Se o dia selecionado não pertence à semana atual, mostrar o primeiro dia da
                    // semana
        ...
        if (blurToggleBtn != null && anotacoesArea != null) {
            blurToggleBtn.setOnAction(e -> {
                ...
                if (anotacoesBlurred) {
                    ...
                    blurToggleBtn.setStyle(
                            "-fx-background-radius: 12; -fx-background-color: rgb(60, 60, 60); -fx-font-size: 15px; -fx-padding: 0; -fx-text-fill: white;");
    ...
    public void mostrarClientes() {
        ...
        if (appController != null
                && (appController.getClientesMap() == null || appController.getClientesMap().isEmpty())) {
            ...
            adicionarBtn.setOnAction(e -> {
                try {
                    ...
                    stage.setX(areaCentral.localToScreen(areaCentral.getBoundsInLocal()).getMinX()
                            + (areaCentral.getWidth() - largura) / 2);
    ...
    private void atualizarTabelaClientes(VBox layout, String filtro) {
        ...
        for (int i = 0; i < cabecalho.length; i++) {
            ...
            th.setStyle(
                    "-fx-font-size: 15px; -fx-font-weight: bold; -fx-background-color: rgba(197, 130, 63, 0.86); -fx-text-fill: white; -fx-background-radius: 12; -fx-padding: 8 0 8 0; -fx-border-width: 0;");
                ...
                for (int col = 0; col < cabecalho.length; col++) {
                    ...
                    switch (col) {
                        case 0:
                            ...
                            break;
                        case 1:
                            texto = c.getNumeroTelefone();
                            break;
                        case 2:
                            texto = c.getTipoCliente().toString();
                            break;
                        case 3:
                            texto = String.valueOf(c.getFaltas());
                            break;
                        case 4:
                            texto = c.getTipoCliente() == models.Cliente.TipoCliente.SEMANAL ? c.getDiaSemana() : "—";
                            break;
                        case 5:
                            texto = c.getTipoCliente() == models.Cliente.TipoCliente.SEMANAL ? c.getHoraCorte() : "—";
                            break;
                        default:
                            texto = "—";
                    }
    ...
    private void atualizarMes() {
        ...
        for (int col = 0; col < 7; col++) {
            ...
            label.setStyle(
                    "-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: white; -fx-background-color: rgba(197, 130, 63, 0.86); -fx-background-radius: 12; -fx-border-width: 0; -fx-padding: 8 0 8 0;");
        ...
        // Descobre o primeiro dia do mês e o primeiro dia a mostrar (pode ser do mês
        // anterior)
        LocalDate primeiroDiaMes = semanaAtual.withDayOfMonth(1);
        ...
        for (int row = 2; row <= 6; row++) {
            for (int col = 0; col < 7; col++) {
                ...
                if (data.equals(LocalDate.now())) {
                    cell.setStyle(
                            "-fx-background-color: rgb(189, 166, 35); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-cursor: hand;");
                    diaLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: white; -fx-font-weight: normal;");
                } else if (utils.Feriados.isFeriado(data)) {
                    cell.setStyle(
                            "-fx-background-color: rgb(36, 43, 141); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-cursor: hand;");
                    diaLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: white; -fx-font-weight: bold;");
                } else if (data.getMonth() != primeiroDiaMes.getMonth()) {
                    cell.setStyle(
                            "-fx-background-color: rgba(43, 40, 40, 0.53); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-cursor: hand;");
                    diaLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: #bbb; -fx-font-weight: normal;");
                } else {
                    cell.setStyle(
                            "-fx-background-color: rgb(43, 40, 40); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-cursor: hand;");
                }
    ...
    private void atualizarDia() {
        ...
        Map<java.time.LocalDateTime, Marcacao> marcacoesMap = (appController != null
                && appController.getMarcacoesMap() != null)
        ...
        while (!horaAtual.isAfter(HORA_FECHO)) {
            ...
            horaLabel.setStyle(
                    "-fx-background-color: rgba(197, 130, 63, 0.86); -fx-text-fill: white; -fx-font-weight: bold; -fx-alignment: center; -fx-border-width: 0; -fx-padding: 8; -fx-font-size: 15px; -fx-background-radius: 12;");
            ...
            boolean isPassado = dataSelecionada.isBefore(LocalDate.now()) ||
                    (dataSelecionada.isEqual(LocalDate.now())
                            && horaSelecionada.isBefore(LocalTime.now().withSecond(0).withNano(0)));
            ...
            if (is15min1 || is15min2) {
                ...
                if (is15min2) {
                    ...
                    espaco2.setOnMouseClicked(
                            e -> abrirCriarMarcacao(dataSelecionada, horaSelecionada.plusMinutes(15)));
            ...
            if (!isPassado || marcacao1 != null) {
                celula.setOnMouseEntered(ev -> celula.setStyle(
                        "-fx-background-color: white; -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40; -fx-cursor: hand;"));
    ...
    private void criarCabecalhoDias() {
        ...
        for (int dia = 0; dia < 7; dia++) {
            ...
            if (dataAtual.equals(hoje)) {
                cabecalhoPane.setStyle(
                        "-fx-background-color:rgb(189, 166, 35); -fx-background-radius: 12; -fx-border-radius: 12; -fx-border-width: 0; -fx-padding: 8;");
            } else if (utils.Feriados.isFeriado(dataAtual)) {
                cabecalhoPane.setStyle(
                        "-fx-background-color:rgb(36, 43, 141); -fx-background-radius: 12; -fx-border-radius: 12; -fx-border-width: 0; -fx-padding: 8;");
            } else {
                cabecalhoPane.setStyle(
                        "-fx-background-color: rgba(197, 130, 63, 0.86); -fx-background-radius: 12; -fx-border-width: 0; -fx-padding: 8;");
            }
        ...
        switch (modoAtual) {
            ...
            case DIA:
                ...
                if (diaSelecionado.equals(LocalDate.now())) {
                    semanaLabel
                            .setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: rgb(189, 166, 35);");
                } else if (Feriados.isFeriado(diaSelecionado)) {
                    semanaLabel
                            .setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: rgb(36, 43, 141);");
                } else {
                    semanaLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: white;");
                }
                break;
    ...
    private void criarGradeHorarios() {
        ...
        while (!horaAtual.isAfter(HORA_FECHO)) {
            ...
            for (int dia = 0; dia < 7; dia++) {
                ...
                if (!isPassado || marcacao1 != null) {
                    if (dia == 6) {
                        celula.setOnMouseEntered(e -> celula.setStyle(
                                "-fx-background-color:rgb(221, 233, 236); -fx-background-radius: 12; -fx-min-height: 40;"));
                        celula.setOnMouseExited(e -> celula.setStyle(
                                "-fx-background-color: rgba(197, 130, 63, 0.86); -fx-border-color: rgba(197, 130, 63, 0.86); "
                                        + "-fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40;"));
                    } else {
                        celula.setOnMouseEntered(e -> celula.setStyle(
                                "-fx-background-color: white; -fx-background-radius: 12; -fx-min-height: 40;"));
                        celula.setOnMouseExited(e -> celula.setStyle(
                                "-fx-background-color: rgb(43, 40, 40); -fx-border-color: rgba(197, 130, 63, 0.86); "
                                        + "-fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40;"));
                    }
                    if (!isPassado) {
                        celula.setOnMouseClicked(e -> abrirCriarMarcacao(dataSelecionada, horaSelecionada));
    ...
    private void destacarBlocoAtual() {
        if (!diaSelecionado.equals(LocalDate.now())) {
            celulasDia.forEach((hora, pane) -> {
                if (diaSelecionado.getDayOfWeek() == DayOfWeek.SUNDAY) {
                    pane.setStyle(
                            "-fx-background-color: rgba(197, 130, 63, 0.86); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40;");
                } else {
                    pane.setStyle(
                            "-fx-background-color: rgb(43, 40, 40); -fx-border-color: rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40;");
                }
            });
            return;
        }
        ...
        celulasDia.forEach((hora, pane) -> {
            if (hora.equals(blocoAtual)) {
                pane.setStyle(
                        "-fx-background-color: rgb(189, 166, 35); -fx-border-color:rgba(197, 130, 63, 0.86); -fx-border-width: 1; -fx-background-radius: 12; -fx-border-radius: 12; -fx-min-height: 40;");
    ...
    public void atualizarBoxClientesPendentes() {
        ...
        for (int i = 0; i < pendentes.size(); i++) {
            ...
            nome.setStyle(
                    "-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: white; -fx-padding: 2 4 2 4; -fx-cursor: hand;");
    ...
    private Pane criarBoxMarcacao(Marcacao marcacao, boolean meiaMarcacao, boolean ladoEsquerdo,
            boolean existeOutraMarcacao) {
            ...
            if (cliente.getTipoCliente() == Cliente.TipoCliente.SEMANAL) {
                ...
                if (ultima == null) {
                    ...
                    if (!hoje.isBefore(limite)) {
                        ...
                        var novas = utils.MarcacoesSemanais.gerarMarcacoesSemanais(cliente, marcacoes,
                                dataUltima.plusWeeks(1));
                        for (Marcacao m : novas)
                            marcacoes.put(m.getDataHora(), m);
                    }
                }
